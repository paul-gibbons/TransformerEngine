...
class _Buffer:
        ...
        for stat in stats:
            if isinstance(stat, str):
                ...
                if parsed is not None:
                    ...
                    stat_count, stat_pct = add_num_zeros_stats(threshold)
                    registered_stat = stat_pct if is_pct else stat_count
                    self._stat_name_mapping[stat] = registered_stat
                    self.stats_to_compute = self.stats_to_compute | DEPENDENCIES[registered_stat]
                    continue
    ...
    def log(self):
        ...
        for stat_name in self.stats_to_log:
            if stat_name in self._stat_name_mapping:
                registered_stat = self._stat_name_mapping[stat_name]
                combiner = STATS[registered_stat][1]
                stat_value = combiner(gathered_helper_stats)
                stat_name_str = registered_stat
            else:
            ...
            output[(self.layer_name, self.tensor_name, stat_name_str, self.iteration)] = (
